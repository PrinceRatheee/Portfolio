...
const ProjectCard = ({
  ...
  return (
    <motion.div className="project_card" variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt
        options={{
          max: 45,
          scale: 1,
      ...
      >
          ...
          <div className='absolute inset-0 flex justify-end m-3 card-img_hover'>
            ...
            >
              <img
                src={github}
                alt='source code'
                className='w-1/2 h-1/2 object-contain cursor-pointer bg-black rounded-full'
              />
            </div>
          </div>
        </div>
        ...
        <div className='mt-4 flex flex-wrap gap-2'>
          {tags.map((tag) => (
            <p
              key={`${name}-${tag.name}`}
              className={`text-[14px] ${tag.color} text-blue-400`}
            >
              #{tag.name}
            </p>
          ))}
